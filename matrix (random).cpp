...
public:
	...
	Matrix(int x, int y, int from, int to) :
		Matrix() {
		Matrix temp(x, y);

		temp.randomize(from, to);
	...
	void setSize(int x, int y, int from, int to) {
		Matrix temp(x, y, from, to);
...
private:
	...
	int randomNumber(int from, int to) {
		...
		return from + abs(static_cast<long long>(rnd())) % (to - from + 1);
	}
};